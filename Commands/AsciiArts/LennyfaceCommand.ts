...
export class LennyfaceCommand implements ISlashCommand {
  public command: string = 'lennyface';
  public i18nDescription: string = 'lennyface_description';
  public i18nParamsExample: string = 'lennyface_params';
  ...
  public async executor(context: SlashCommandContext, read: IRead, modify: IModify, http: IHttp, persis: IPersistence): Promise<void> {
    ...
    await sendMessageToRoom({
      ...
      text: message.length ? `${AsciiArtCommand.LENNYFACE} ${message}` : AsciiArtCommand.LENNYFACE,
    });
  }
}
